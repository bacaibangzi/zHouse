...
;(function($,window){
	...
	var exportsMethods = {
		...
		newVerScrollForPull : function(wrapper,pulldownAction,pullupAction,opts,pullText){
			...
			var pulldownRefresh   = pullText && pullText['pulldownRefresh'] ? pullText['pulldownRefresh'] : '下拉刷新报表...',
				pullupLoadingMore = pullText && pullText['pullupLoadingMore'] ? pullText['pullupLoadingMore'] : '上拉加载更多数据...',
			...
			scrollObj.on('scrollMove',function(){
				...
				} else if ($pullup.length>0 && this.y < this.minScrollY && this.y < (this.maxScrollY - 55) && !$pullup.hasClass('flip')) {
