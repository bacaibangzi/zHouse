...
<body>
    ...
    <c:if test="${vo.orgType ==1}">
    	<div class="grid_summay_opts" id="btns">
				<div class="left_opts">
						<a hideFocus="true" href="javascript:add('<%=basePath%>disOrg/eidt.htm?code=${vo.code}&orgType=${vo.orgType}')" class="gray_radiu_btn" >
							<em class="gray_l"></em> 
							<em class="gray_r"></em> 
							新增
						</a>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			var tableObj={}
			tableObj.url="<%=basePath%>disOrg/list.htm?code=${vo.code}";
			...
			tableObj.aoColumns=[
				...
				{"mDataProp":"dqDate","sDefaultContent": "","bSortable":false,"sClass":"text-align-mid","sWidth":"100"}
			];
			//添加额外的参数
		...
		function update()
		{
			...
			window.location = "<%=basePath%>disOrg/eidt.htm?code=${vo.code}&orgCode="+orgCode+"&entityId="+selectId+"&orgType="+orgType+"&code1=${vo.orgType}";
		}
		/** 查看详情 **/
		function detail()
		{
			...
			var selectId = data.idStr;
			// 明细弹出框
			var commonDialog = commonOpenDialog("readDetail",'详细',700,450, '<%=basePath%>disOrg/detail.htm?code=${vo.code}&entityId=' + selectId);
		...
		function highQuery()
		{
			...
			var returnVal = '<%=basePath%>disOrg/list.htm?code=${vo.code}';
		...
		function deleteAll()
		{
			...
			var flag = null;
			$.ajax({
				url:'<%=basePath%>disOrg/checkIfCanbeDeleted.htm',
				type:'post',
				async: false,
				data:'entityIds='+ids,
			...
			var rows=getSeleteObjs(datatable);
			var actionUrl = "<%=basePath%>disOrg/delete.htm?code=${vo.code}&orgType=${vo.orgType}&entityIds=";
