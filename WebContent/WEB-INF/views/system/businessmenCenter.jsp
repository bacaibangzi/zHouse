...
<body>
    ...
    <div class="contenbox">
    <c:if test="${vo.orgType >1 and vo.orgType<4 or vo.orgType>6}">
    	<div class="grid_summay_opts" id="btns">
				<div class="left_opts">
						<a hideFocus="true" href="javascript:add('<%=basePath%>businessmen/eidt.htm?orgCode=${vo.orgCode}&orgType=${vo.orgType}')" class="gray_radiu_btn" >
							<em class="gray_l"></em> 
							<em class="gray_r"></em> 
							新增
						</a>
        ...
        <div class="rightMenu bodyMenu Js_tdMenu">
            ...
            <ul>
                ...
                <li><a hideFocus="true" href="javascript:disRole()">分配商户服务</a></li>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			var tableObj={}
			//tableObj.url="<%=basePath%>businessmen/list.htm?orgCode=${vo.orgCode}";
		...
		function update()
		{
			...
			window.location = "<%=basePath%>businessmen/eidt.htm?orgCode=${vo.orgCode}&entityId="+selectId+"&orgType=${vo.orgType}";
		}
		/** 查看详情 **/
		function detail()
		{
			...
			var selectId = data.idStr;
			// 明细弹出框
			var commonDialog = commonOpenDialog("readDetail",'详细',700,450, '<%=basePath%>businessmen/detail.htm?orgCode=${vo.orgCode}&entityId=' + selectId);
		...
		function highQuery()
		{
			...
			var returnVal = '<%=basePath%>businessmen/list.htm?orgCode=${vo.orgCode}&code=3';
		...
		function deleteAll()
		{
			...
			var actionUrl = "<%=basePath%>businessmen/delete.htm?orgCode=${vo.orgCode}&entityIds=";
		...
		{
			var rows=getSeleteObjs(datatable);
			var actionUrl = "<%=basePath%>businessmen/sh.htm?orgCode=${vo.orgCode}&value=1&entityIds=";
		...
		{
			var rows=getSeleteObjs(datatable);
			var actionUrl = "<%=basePath%>businessmen/sh.htm?orgCode=${vo.orgCode}&value=0&entityIds=";
		...
		{
			var rows=getSeleteObjs(datatable);
			var actionUrl = "<%=basePath%>businessmen/sh.htm?orgCode=${vo.orgCode}&value=9&entityIds=";
		...
		function disRole()
		{
			...
			var selectId = data.idStr;
			// 明细弹出框
			var commonDialog = commonOpenDialog("readDetail",'分配商户服务',700,450, '<%=basePath %>merchantserServer/rightTree.htm?orgCode=${orgCode}&id=' + selectId,null);
