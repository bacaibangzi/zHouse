...
<body>
    ...
    <div class="contenbox">
    <c:if test="${vo.orgType ==2 or vo.orgType==3 or vo.orgType==9}">
    	<div class="grid_summay_opts" id="btns">
				<div class="left_opts">
						<a hideFocus="true" href="javascript:add('<%=basePath%>accountOrg/eidt.htm?code=${vo.code}&orgType=${vo.orgType}&code1=${vo.code1}')" class="gray_radiu_btn" >
							<em class="gray_l"></em> 
							<em class="gray_r"></em> 
							新增
						</a>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			var tableObj={}
			tableObj.url="<%=basePath%>accountOrg/list.htm?code=${vo.code}";
		...
		function update()
		{
			...
			window.location = "<%=basePath%>accountOrg/eidt.htm?code=${vo.code}&orgCode="+orgCode+"&entityId="+selectId+"&orgType="+orgType+"&code1=${vo.orgType}"; 
		}
		/** 查看详情 **/
		function detail()
		{
			...
			var selectId = data.idStr;
			// 明细弹出框
			var commonDialog = commonOpenDialog("readDetail",'详细',700,450, '<%=basePath%>accountOrg/detail.htm?code=${vo.code}&entityId=' + selectId);
		...
		function highQuery()
		{
			...
			var returnVal = '<%=basePath%>accountOrg/list.htm?code=${vo.code}';
		...
		function deleteAll()
		{
			...
			var flag = null;
			$.ajax({
				url:'<%=basePath%>accountOrg/checkIfCanbeDeleted.htm',
				type:'post',
				async: false,
				data:'entityIds='+ids,
			...
			var actionUrl = "<%=basePath%>accountOrg/delete.htm?orgType=${vo.orgType}&code=${vo.code}&entityIds=";
