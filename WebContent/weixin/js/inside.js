...
G.ui.tips = {
    ...
    html2:function(t, u, w, h){
    	if (!w) w = 500;
        if (!h) h = 300;
        $.fallr('show', {
            content: '<h2>' + t + '</h2>' + '<img src='+u+' alt="" />',
            width: w + 130,
            icon: null,
            closeOverlay: true,
            position: 'center',
    ...
    tip: function (data) {
        if (data.errno == 0) {
            if (data.tip) {
            	...
            	G.ui.tips.html2("扫一扫", data.preview,"420px","500px")
            }else if(data.debug){
                        ...
                        if (typeof KindEditor !== "undefined" && KindEditor.instances) {
                            $.each(KindEditor.instances, function () {
                                ...
                                if (e) {
                                    ...
                                    if (!$element.prop("disabled")&&len <= min || len >= max) {
    ...
    wizard: function () {
        $(".form-wizard").length > 0 && $(".form-wizard").formwizard({
            ...
            validationOptions: {
                ...
                errorPlacement: function (e, t) {
                	if(t.data("ruleSums")){
                		t.parents(".controls").find(".js_sums_list_error").remove();
                		e.addClass('js_sums_list_error')
                	}
                ...
                success: function (e) {
                    e.addClass("valid").closest(".control-group").removeClass("error success");
                    var $p=e.parents(".controls");
                    if(e.hasClass('js_sums_list_error')){
                    	$p.find(".js_sums_list_error").css('display',"none")
                    }
                }
            },
            formOptions: {
...
G.logic.page = {
    ...
    common: function () {
        ...
        if ($daterangepick.length > 0) {
            $daterangepick.each(function () {
                ...
                maxid=$this.data("maxdateid"),maxdate=$this.data("maxdate"),even = $(this).data("change");
                var timef=true,formats="YYYY/MM/DD HH:mm",day=$this.data("day");
                if(day){
                	timef=false;
                	formats="YYYY/MM/DD";
                }
        ...
        $(".datepick").length > 0 && $(".datepick").each(function(){
    ...
    copy: function () {
        ...
        $(".copy_text").each(function (i) {
        	var texttips=$(this).data("msg") || "复制成功,请粘帖到您需要的地方";
            var $id = "copy_button{0}".format(i),
				_tmp = '  <button class="btn copy" id="{0}" type="button" data-clipboard-target="{2}" data-clipboard-text="{1}"><i class="icon-copy"><\/i>复制<\/button>  <span class="alert copy-success help-inline alert-success hide "  >'+texttips+'<\/span>',
				$this = $(this),
				v = $this.text(),
				$iid = $this.attr("id");
