...
$(document).ready(function(){
	...
	if(url=="/CateSystem/weiXinCS/shopItem-weixin"){
	/**
	获取商品类别和商品
	**/
	$("#loading").show();
	...
	$.ajax({
		url:"../weiXinCS/getShopItem-weixin",
		method:"get",
		...
		$("#afterLoading").show();
			...
			var html1 = "<li id='li_type' class='on'>所有商品</li>";
			var html2="";
			var shopItemSort = data.shopItemSort;
			if(shopItemSort.length>0){
				var allmoney = 0;
				var menucount = 0;
				for(var i=0;i<shopItemSort.length;i++){
					html1 = html1 + "<li id='li_type"+shopItemSort[i].no+"' class=''>"+shopItemSort[i].name+"</li>";
					var htmlBegin = "htmlBegin"+shopItemSort[i].no;
					window['htmlBegin'] = "<ul id='ul_type"+shopItemSort[i].no+"'>";
					var htmlEnd = "</ul>";
					var item = "shopItems"+shopItemSort[i].no;
					var itemList="itemList"+shopItemSort[i].no;
					window['itemList']= "";
					var dishInfos = data[item];
					for(var j=0;j<dishInfos.length;j++){
						//设置默认图片
						var picture="../common/images/nopic.jpg";
						if(dishInfos[j].pic!=null&&dishInfos[j].pic!==""){
							picture=basePath+dishInfos[j].pic;
						}
						
						if(dishInfos[j].number==0){
							window['itemList'] = window['itemList'] + "<li id='li"+dishInfos[j].barcode+"' class=''><span><img src='"+picture+"' alt='商品简介："+dishInfos[j].guige+"'>"
							  + "</span><label><span id='diVchDishName'>"+dishInfos[j].name+"</span><label><span class='icon hart' data="+dishInfos[j].itemSortNo+"></span><span class='price'>￥"
							  + "<label class='price_n'>"+dishInfos[j].price+"</label></span><label><span class='btn  minus' style='display: none;'></span><span class='num'>"
							  + "<input type='text' readonly='true' value='"+dishInfos[j].number+"' style='display: none;'></span><span class='btn plus'></span></label></label></label><input id='sn' type='hidden' value='"+dishInfos[j].sn+"'></li>";
						}else{
							window['itemList'] = window['itemList'] + "<li id='li"+dishInfos[j].barcode+"' class=''><span><img src='"+picture+"' alt='商品简介："+dishInfos[j].guige+"'>"
							  + "</span><label><span id='diVchDishName'>"+dishInfos[j].name+"</span><label><span class='icon hart' data="+dishInfos[j].itemSortNo+"></span><span class='price'>￥"
							  + "<label class='price_n'>"+dishInfos[j].price+"</label></span><label><span class='btn  minus' ></span><span class='num'>"
						...
						allmoney = allmoney + (dishInfos[j].price)*(dishInfos[j].number);
						menucount = menucount + dishInfos[j].number;
						
					}
						html2 = html2 + window['htmlBegin'] + window['itemList'] + htmlEnd;
				}
	...
	//获取购物车菜品
	}else if(url=="/CateSystem/weiXinCS/shopItemConfirm-weixin"){
		var orgCode = $("#orgCode").val();
		var allmoney = 0;
		var menucount = 0;
		$.ajax({
			url:"../weiXinCS/getCartItem-weixin",
			method:"get",
			...
			if(data.length>0){
				for(var i=0;i<data.length;i++){
					...
					*/
					html = html + "<li id='"+data[i].barcode+"'><span><img src="+picture+"></span><label><span id='diVchDishName'>"+data[i].dishName+"</span><label><span class='price'>￥"
								+ "<label class='price_n'>"+data[i].price+"</label></span><label><span class='btn minus' onclick='minus(this)'></span><span class='num'>"
								+ "<input type='text' value="+data[i].number+" readonly='true'></span><span class='btn plus' onclick='plus(this)'></span></label></label>"		
								+ "</label><input type='text' class='markinput' placeholder='备注' value=''>	<input id='sn' type='hidden' value='"+data[i].sn+"'></li>";
					//}
