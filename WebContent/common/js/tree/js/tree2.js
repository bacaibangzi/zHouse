...
(function($) {
    ...
    $.fn.treeview = function(settings) {
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            if( nd.icon){
            	for(var i = 0 ; i<nd.icon.length;i++){
            	   //title=''
            	   ht.push("&nbsp;<img style='vertical-align: middle;'  src='" + nd.icon[i].icon + "' title='" + nd.icon[i].title + "'/>");
            	}
            }
            ht.push("</span>");
            ht.push("</a>");
        ...
        function getckdd(items, c, fn, deep) {
            if(dfop.displayDeep && deep >= dfop.displayDeep){// 增加显示级别设置的节点过滤
                return false;
            }
            for (var i = 0, l = items.length; i < l; i++) {
                if((items[i].showcheck == true && items[i].checkstate == 1) && c.push(fn(items[i]))){
                	continue;
                }
                if (items[i].ChildNodes != null && items[i].ChildNodes.length > 0) {
                    getckdd(items[i].ChildNodes, c, fn, deep + 1);
                }
            }
        }
        ...
        function getCKIds(items, c, fn, deep){
            if(dfop.displayDeep && deep >= dfop.displayDeep){// 增加显示级别设置的节点过滤
                return false;
            }
            var item;
            for (var i = 0, l = items.length; i < l; i++) {
                ...
                if (item.showcheck == true && (item.checkstate == 1)){
                    if(!dfop.cascadecheck || item.complete){
                        c.singleNode.push(fn(item));
                    }else{
                        if(checkSelfOrChildrenHasSelected(me[0].t, item.id)){
...
//            }
        ...
        me[0].t = {
            ...
            getCKSelectedIds:function(){
                var initSelectedNode = dfop.aHasSelected;
                var s = {singleNode:[], multiNode:[]};
                getckdd(treenodes, s.singleNode, function(item) { return item.id; }, 0);
                return s;
            },
    ...
    $.fn.getCKIds = function(){
    	if(this[0].t){
			return this[0].t.getCKSelectedIds();
    	}
    	return null;
    }
...
function getTreeCKNodeIds(dialogObj, treeDivId)
{
    // 获取树所有选中的节点ID
    var selectedIdArray = $("#" + treeDivId, dialogObj.dgDoc).getCKIds();
    var selectedIds ="";
    $.each(selectedIdArray,function(i,item){
        if(item != ''){
            selectedIds += item;
