...
(function($) {
	...
	$.fn.extend({//扩展添加新的方法
		autocomplete : function(urlOrData, options) {
			var isUrl = typeof urlOrData == "string";// urlDate是否是url，true：是
			//扩展options对象，用来接收，$.Autocompleter.defaults对象，{url，data，delay，max}对象，和options对象
			options = $.extend({}, $.Autocompleter.defaults, {
				// 将urlDate赋值给url或date
				url : isUrl ? urlOrData : null,
				data : isUrl ? null : urlOrData,
				delay : isUrl ? $.Autocompleter.defaults.delay : 10,// 延时
				max : options && !options.scroll ? 10 : 150
			}, options);

			// if highlight is set to false, replace it with a do-nothing function。hightlight属性若为false，什么都不做
			// 
	...
	$.Autocompleter = function(input, options) {
		...
		$input.bind(
			...
			// @hack by zhouxiaohong for firefox 中文输入支持
			onChange(0, true);
		});
		...
		function selectCurrent() {
			...
			//v = v.substring(0,v.lastIndexOf (options.multipleSeparator));
			$input.val(v);
			hideResultsNow();
		...
		function onChange(crap, skipPrevCheck) {
			...
			if (currentValue.length >= options.minChars) {
				...
				stopLoading();
				select.hide();
				// add by meiguiyang 20111013
				// 当前值小于最小的字符个数时,执行 clearResult方法
				if (options.clearResult) {
					options.clearResult();
				}
				// add by meiguiyang 20111013
	...
	$.Autocompleter.defaults = {
		...
		multiple : false,
		multipleSeparator : ", ",
		highlight : function(value, term) {// 使显著，加亮关键字
	...
	$.Autocompleter.Select = function(options, input, select, config) {
		...
		// 向上翻页
		function uppage2() {
			$("#left2_" + options.tableid).click(function(event) {
				options.page--;
				fillListforPage2(options.page, options.max);
				input.focus();
			});
		...
		// 向下翻页
		function downpage2() {
			// alert($("#right2_"+options.tableid).val())
			$("#right2_" + options.tableid).click(function(event) {
				options.page++;
				fillListforPage2(options.page, options.max);
				input.focus();
			});
		...
		function changeRowsPerPage2() {
			$("#rowsPerPage2_" + options.tableid).change(function(event) {
				options.max = $(this).val();
				fillList();
				input.focus();
			});
		}
		...
		// foot空白处点击后事件
		function footclick() {
			$("#foot_" + options.tableid).click(foo);
		}
		function fillListforPage2(page, rows) {
			if (options.page == 1) {
				$("#left2_" + options.tableid).attr("disabled", true);
			} else if (options.page != 1) {
				$("#left2_" + options.tableid).attr("disabled", false);
			}

			if (options.page * options.max >= data.length) {
				$("#right2_" + options.tableid).attr("disabled", true);
			} else if (options.page * options.max < data.length) {
				$("#right2_" + options.tableid).attr("disabled", false);
			}
			$("#pageshow2_" + options.tableid).attr("value", options.page);
			...
			for ( var i = start; i < end; i++) {
				...
				if (formatted === false)
					continue;
				var tr = $("<tr/>").html(options.highlight(formatted, term))
		...
		// Create results
		function init() {// 初始化，只执行一次
			if (!needsInit)
				return;
			element = $("<table></table>").hide()
					.addClass(options.resultsClass).css("position", "absolute")
					.appendTo(document.body);

			$(options.headTitle).appendTo(element);
					...
					.mouseover(
							function(event) {
								if (target(event).nodeName
										&& target(event).nodeName.toUpperCase() == 'TR') {
									active = $("tr", list).removeClass(
											CLASSES.ACTIVE)
											.index(target(event));
									$(target(event)).addClass(CLASSES.ACTIVE);
								}
							}).click(function(event) {// ul点击事件
						$(target(event)).addClass(CLASSES.ACTIVE);
						select();
			...
			var centerwidth = options.width - 145;// 中间div宽度为了让鼠标点击后触发input.foucse()事件
			if(options.hasFoot){
				
				foot = $(
						"<tfoot><tr class=\"foot_tr\">"
								+ "<td class=\"foot_td\" style=\"width:"
								+ options.width
								+ ";\" colspan=100>"
								+ "<div style=\"left:"+centerwidth+";bottom:2;position:absolute;z-index:2;\">"
								+ "<font>显示条数：</font>"
								+ " <select style=\"width:60;\" id=\"rowsPerPage2_"
								+ options.tableid
								+ "\" >"
								+ "<option>"
								+ options.max
								...
								+ "</select>"
								+ "</div>"
								+ "<div class=\"foot_div\" style=\"width:"
								+ options.width
								+ ";\" id=\"foot_"
								+ options.tableid
								+ "\">"
								...
								+ "<input id=\"left2_"
								+ options.tableid
								+ "\" type=\"button\" value=\"<上一页\" class=\"foot_left2\" />"
								+ "<input id=\"pageshow2_"
								+ options.tableid
								+ "\"  type=\"text\" value=\"1\" class=\"foot_pageshow2\"  disabled=true />"
								+ "<input id=\"right2_"
								+ options.tableid
								+ "\" type=\"button\" value=\"下一页>\" class=\"foot_right2\"  />"
								+ "</div>" + "</td></tr></tfoot>")
		...
		function fillList() {
			...
			$("#pageshow2_" + options.tableid).attr("value", options.page);// 显示页数
			$("#left2_" + options.tableid).attr("disabled", true);
			if (options.page * options.max >= data.length) {
				$("#right2_" + options.tableid).attr("disabled", true);
			} else if (options.page * options.max < data.length) {
			...
			for ( var i = 0; i < max; i++) {
				...
				if (formatted === false)
					continue;
				var tr = $("<tr/>").html(options.highlight(formatted, term))// 设置每个<tr/>的类容
