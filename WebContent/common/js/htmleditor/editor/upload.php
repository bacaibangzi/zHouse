...
if($_GET['action']!='upload'){
	...
	if(!in_array(substr($_FILES['file1']['name'],-3,3),$fileType))
		die("<script>alert('不允许上传该类型的文件！-808');window.parent.document.getElementById('divProcessing').style.display='none';history.back();</script>");
	if(strpos($_FILES['file1']['type'],'image')===false)
		die("<script>alert('不允许上传该类型的文件！');window.parent.document.getElementById('divProcessing').style.display='none';history.back();</script>");
	if($_FILES['file1']['size']> $maxSize*1024)
		die( "<script>alert('文件过大！');window.parent.document.getElementById('divProcessing').style.display='none';history.back();</script>");
	if($_FILES['file1']['error'] !=0)
		die("<script>alert('未知错误，文件上传失败！');window.parent.document.getElementById('divProcessing').style.display='none';history.back();</script>");
