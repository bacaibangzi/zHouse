 ...
 */
	...
	this.show = function(obj){
		if($("#message").is("div")){ return; }			if(obj.width)this.layer.width = obj.width;;				if(obj.height) this.layer.height = obj.height ;
		if(!obj.title)obj.title = this.title;
		this.inits(obj.title, obj.text);
		if(obj.time>=0)this.time = obj.time;
		...
		}		//setTimeout(function(){this.close();},this.time)
	...
	this.close = function(){
		switch(this.anims.type){
			case 'slide':$("#message").slideUp(this.anims.speed,function(){$("#message").remove();});break;
