/*
 * ! jQuery Form Plugin version: 3.02 (07-MAR-2012) @requires jQuery v1.3.2 or
 * later
 * 
...
(function($) {
	...
	$.fn.ajaxSubmit = function(options) {
		...
		var fileInputs = $('input:file:enabled[value]', this); // [value]
																// (issue #113)
					...
					if (xhr.upload) {
						xhr.upload.onprogress = function(event) {
							...
							if (event.lengthComputable)
								percent = parseInt(
										(event.position / event.total) * 100,
										10);
							options.uploadProgress(event, event.position,
									event.total, percent);
						}
					}
					return xhr;
