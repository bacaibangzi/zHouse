...
(function () {
    ...
    Editor.prototype = /**@lends baidu.editor.Editor.prototype*/{
        ...
        setContent : function ( html,notFireSelectionchange) {
            ...
            //去掉了\t\n\r 如果有插入的代码，在源码切会所见即所得模式时，换行都丢掉了
            this.document.body.innerHTML = html.replace(new RegExp('[' + domUtils.fillChar + ']*','g'),'');
...
//                            'border:'+(ci.border||'0') + 'px solid #000; ' +
//                            'margin:'+(ci.margin || '0') + 'px;" ' +
                    ...
                    for(var i=0;ci=opt[i++];){
                        str =  '<p ' + (ci['floatStyle'] == 'center' ? 'style="text-align: center" ' : '') + '><img src="'+ci.src+'" '+
                            (ci.width ? 'width="'+ci.width+'" ':'') +  ' data_ue_src="' + ci.data_ue_src + '" ' +
                            (ci.height ? ' height="'+ci.height+'" ':'') +
...
(function (){
    ...
    baidu.editor.commands['indent'] = {
        execCommand : function() {
             ...
             me.execCommand('Paragraph','p',{'text-indent': value});
        },
        queryCommandState : function() {
...
(function(){
    ...
    baidu.editor.commands['lineheight'] =  {
        execCommand : function( cmdName,value ) {
            this.execCommand('paragraph','p',{'line-height' : value + '%' });
            return true;
        },
        queryCommandValue : function() {
            var startNode = this.selection.getStart(),
...
baidu.editor.commands["wordcount"]={
    queryCommandValue:function(cmd,onlyCount){
        ...
        if(onlyCount){
            ...
            return length = contentText.length;
        }
        ...
        me.addListener('keyup', function(type, evt) {
            ...
            if (keyCode == 8 || keyCode == 46) {
                ...
                if (domUtils.isEmptyBlock(me.body) && !range.startOffset) {
                    ...
                    me.body.innerHTML = '<p>'+(browser.ie?'':'<br/>')+'</p>';
                    range.setStart(me.body.firstChild,0).setCursor();
                    me.undoManger && me.undoManger.save();
                    return;
                }
...
(function() {
    ...
    baidu.editor.plugins['autoheight'] = function() {
        ...
        editor.enableAutoHeight = function (){
            ...
            editor.addListener('destory',function(){
                clearInterval(timer)
            });
